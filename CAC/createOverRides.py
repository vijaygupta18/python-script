...
table_names = ['driver_pool_config'] # NOTE: This works only for one table at a time!!!! (I have used list here as this part was copied from sqlToCac.py  Lol :-P )
env = 'dev'
app = 'dobpp'
  ...
  for i in range(len(dist_overrides[ind])):
    ...
    if (overrides[ind][1] == '=='):
      ...
      if dist_overrides[ind][i] != None and dist_overrides[ind][i] != "None": 
        context.append({"==":[{"var":convertOneToCamelCase(overrides[ind][0])}, str(dist_overrides[ind][i])]}) # None is getting ignored by cac
      else: flg = False
