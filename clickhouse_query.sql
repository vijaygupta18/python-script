...
CREATE TABLE atlas_driver_offer_bpp_helper.fare_policy_slabs_details_slab ON CLUSTER `{cluster}`
    ...
    `platform_fee_sgst` Nullable (String),
    `waiting_pickup_charges` Nullable (String)
	)
...
(
	...
	`platform_fee_sgst` String,
	`waiting_pickup_charges` String,
)
	AS SELECT
	ifNull(JSONExtractInt(message,'id'), 0) as id,
	ifNull(JSONExtractString(message,'fare_policy_id'),'') as fare_policy_id,
	...
	ifNull(JSONExtractString(message,'platform_fee_sgst'),'') as platform_fee_sgst,
	ifNull(JSONExtractString(message,'waiting_pickup_charges'),'') as waiting_pickup_charges,
