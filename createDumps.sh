...
# --------------------------------------------
# use this after creating dumps to copy them to local
# run script

# tar -czf dumps.tar.gz ~/dumps

# scp -i ~/internal-jump.pem ubuntu@172.31.34.96:/home/ubuntu/dumps.tar.gz  ./

# scp -P 37622 vijay-gupta@3.111.24.82:/home/vijay-gupta/dumps.tar.gz /Users/vijay.gupta/Desktop
...
for table in $driver_tables; do
    ...
    output_file="dumps/$table.sql"
    mkdir -p "$(dirname "$output_file")"
    pg_dump --host "$host" --username "$username" --dbname "$dbname" --table "$schemaName.$table" --data-only --column-inserts | head -n $((rows_to_export + 2)) > "$output_file"
